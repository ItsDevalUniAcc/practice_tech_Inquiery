...
def show_instructions(screen):
    ...
    heading_font = pygame.font.SysFont('Georgia', 32, bold=True)
    ...
    content_height = 700  # Make it taller to fit all instructions
    ...
    # How to Play heading
    how_to_play_text = heading_font.render("How to Play Football Quiz", True, BLACK)
    ...
    instructions = [
        "Welcome to the Football Math Quiz Challenge!",
        "",
        "How to Play:",
        "- You will be presented with math questions to solve quickly",
        "- Type your answer using the keyboard and press Enter to shoot",
        "- If your answer is correct, you will score a goal!",
        "- If your answer is wrong or you run out of time, you'll miss the shot",
        "- You have 8 seconds to answer each question",
        "- There are 8 questions in total to test your math skills",
        "",
        "Scoring:",
        "- Each correct answer (goal) earns you 1 point",
        "- Your goal is to score as many points as possible",
        "- If you miss a shot, the game is over",
        "- Complete all 8 questions correctly to win the game",
        "",
        "Controls:",
        "- Use the keyboard to type your answer",
        "- Press Enter to submit your answer and shoot",
        "- Use left and right arrow keys to move the player",
        "",
        "Tips:",
        "- Calculate quickly but accurately",
        "- Keep an eye on the timer - you only have 8 seconds per question!",
        "- Focus on getting the math correct to score goals",
        "",
        "Are you ready to test your math skills on the football field? Click Play Now to begin!"
    ]
    ...
    while running:
        ...
        # Title
        title_text = title_font.render("FOOTBALL QUIZ", True, WHITE)
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Football Quiz - Instructions")

    result = show_instructions(screen)
    print(f"Start game: {result}")  # Debug output to confirm return value

    if result:
        # User clicked Play Now, launch the game
        pygame.quit()  # Close this window first
        import os
        os.system(f"{sys.executable} footballquizl1.py")  # Start the game
    else:
        # User chose to quit
        pygame.quit()
        sys.exit()