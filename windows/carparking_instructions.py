...
def show_instructions(screen):
    ...
    # How to Play heading
    how_to_play_text = heading_font.render("How to Play Car Parking Puzzle", True, BLACK)
    ...
    instructions = [
        "Car Parking Puzzle is a sliding block puzzle game where your goal",
        "is to move the red car to the exit.",
        "",
        "How to Play:",
        "- Click on a car to select it",
        "- Click again in the direction you want to move the car",
        "- Cars can only move forward or backward in their orientation",
        "- The red car is your player car - get it to the exit (pink area)",
        "- Try to solve the puzzle in as few moves as possible",
        "",
        "Controls:",
        "- Mouse: Click on cars to select and move them",
        "- Arrow Keys: Move the selected car",
        "- R: Reset the level",
        "- U: Undo your last move",
        "",
        "Scoring:",
        "- Your score is based on the number of moves you make",
        "- Fewer moves = more stars",
        "- 3 stars: Perfect solution",
        "- 2 stars: Good solution",
        "- 1 star: Completed but can improve",
        "",
        "Tips:",
        "- Plan your moves carefully",
        "- Sometimes you need to move other cars to clear a path",
        "- Think about the order of moves to minimize the total count",
        "",
        "Are you ready to solve the car parking challenge?"
    ]

    y_pos = 80
    line_spacing = 32  # Spacing between lines
    ...
    while running:
        ...
        # Title
        title_text = title_font.render("Car Parking Puzzle", True, WHITE)
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Car Parking Puzzle - Instructions")
    ...
    if result:
        ...
        import os
        os.system(f"{sys.executable} carparkingl1.py")  # Start the game
    else:
        # User chose to quit
        pygame.quit()
        sys.exit()