...
def show_instructions(screen):
    ...
    # How to Play heading
    how_to_play_text = heading_font.render("How to Play Word Builder", True, BLACK)
    ...
    instructions = [
        ...
        "- All words belong to a category shown at the top.",
        "- Click the letters in the correct order.",
        "- Use the Submit button to check your answer.",
        "- You can press 'Backspace' to remove a letter.",
        "- Use the Hint button to get a clue (reduces score).",
        "",
        "Scoring Rules:",
        "- You get 10 points for a correct word without hint.",
        "- You get 5 points if you used a hint.",
        "- You have only 130 seconds to solve as many words as possible.",
        "- The game ends when time runs out or you make a mistake.",
        "",
        "Controls:",
        "- Left mouse button: Click on letters, buttons",
        ...
        "- School Supplies (pen, pencil, ruler, etc.)",
        "- Food (bread, rice, egg, etc.)",
        "- Nature (sun, moon, tree, etc.)"
    ]
    ...
    while running:
        ...
        # Title
        title_text = title_font.render("Word Builder Game", True, WHITE)
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Word Builder - Instructions")

    result = show_instructions(screen)
    print(f"Start game: {result}")  # Debug output to confirm return value

    if result:
        # User clicked Play Now, launch the word builder game
        pygame.quit()  # Close this window first
        import os
        os.system(f"{sys.executable} wordbuilderl1.py")  # Start the game
    else:
        # User chose to quit
        pygame.quit()
        sys.exit()