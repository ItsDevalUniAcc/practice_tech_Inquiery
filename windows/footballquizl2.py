...
DIFFICULTY = "MEDIUM"  # Medium difficulty for level 2


# Question generation
def make_question():
    # For level 2, include more complex addition/subtraction and basic multiplication
    if random.random() < 0.8:                      # 80% chance of add and sub problems
        a, b = random.randint(10, 90), random.randint(10, 50)
        op = random.choice(["+", "-"])
        txt = f"{a} {op} {b} = ?"
        ans = eval(f"{a}{op}{b}")
    else:                                          # 20% chance of multiplication problem
