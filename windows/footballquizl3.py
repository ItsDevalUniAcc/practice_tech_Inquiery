...
QUESTION_TIME = 7  # seconds (reduced from level 2 to increase difficulty)
QUESTION_TOTAL = 8  # number of questions
TIP_MS = 1000
SCORE_FILE = "highscores.json"
DIFFICULTY = "HARD"  # Hard difficulty for level 3


# Question generation
def make_question():
    # For level 3, include complex addition/subtraction and more multiplication
    if random.random() < 0.7:  # 70% chance of add and sub problems
        a, b = random.randint(10, 99), random.randint(10, 99)
        op = random.choice(["+", "-"])
        txt = f"{a} {op} {b} = ?"
        ans = eval(f"{a}{op}{b}")
    else:  # 30% chance of multiplication problem
        a, b = random.randint(0, 10), random.randint(0, 50)
        txt = f"{a} * {b} = ?"
        ans = a * b
    return txt, ans
...
def main_game():
    ...
    # For level 3, add distraction balls that move randomly
    distraction_balls = []
    for _ in range(3):  # 3 distraction balls moving around
        distraction_balls.append({
            'rect': pygame.Rect(random.randint(100, WIDTH - 100), random.randint(200, HEIGHT - 200), 15, 15),
    ...
    while True:
        ...
        # Update distraction balls - only in level 3
        for ball_data in distraction_balls:
            ball_data['rect'].x += ball_data['vx']
            ball_data['rect'].y += ball_data['vy']

            # Bounce off walls
            if ball_data['rect'].left <= 0 or ball_data['rect'].right >= WIDTH:
                ball_data['vx'] *= -1
            if ball_data['rect'].top <= 0 or ball_data['rect'].bottom >= HEIGHT:
                ball_data['vy'] *= -1
        ...
        if shot_res is not None:
            ...
            if ball.bottom <= GOAL_Y + 12:
                ...
                # Reset distraction balls
                distraction_balls = []
                for _ in range(3):
                    distraction_balls.append({
                        'rect': pygame.Rect(random.randint(100, WIDTH - 100), random.randint(200, HEIGHT - 200), 15,
                                            15),
                        'vx': random.choice([-2, -1, 1, 2]),
                        'vy': random.choice([-2, -1, 1, 2])
                    })
        ...
        # Draw the distraction balls (only in level 3)
        for ball_data in distraction_balls:
            if random.random() < 0.05:  # 5% chance to change direction randomly
                ball_data['vx'] = random.choice([-2, -1, 1, 2])
                ball_data['vy'] = random.choice([-2, -1, 1, 2])
            pygame.draw.ellipse(scr, (random.randint(200, 255), random.randint(200, 255), random.randint(200, 255)),
                                ball_data['rect'])
        ...
        # Level indicator (only in level 3)
        level_rect = pygame.Rect(WIDTH - 190, 10, 180, 40)
        pygame.draw.rect(scr, (0, 0, 0), level_rect, border_radius=6)
        pygame.draw.rect(scr, (255, 255, 255), level_rect, 2, border_radius=6)
        scr.blit(fSm.render(f"Level: {DIFFICULTY}", True, (255, 100, 100)), (WIDTH - 180, 18))
